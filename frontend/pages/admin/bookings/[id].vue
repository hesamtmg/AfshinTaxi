<template>
  <div style="direction: rtl">
    ...
    <div v-else-if="!booking" class="text-center pa-16">
      ...
      <v-btn class="mt-4" to="/admin/bookings" color="primary" variant="tonal"
        >بازگشت به لیست</v-btn
      >
    </div>
    ...
    <v-row v-else>
      ...
      <v-col cols="12" md="8">
            ...
            <v-col cols="12" sm="6">
              <v-list lines="two" density="compact">
                <v-list-item prepend-icon="mdi-calendar-clock">
                  ...
                  <template #subtitle>
                    <span class="font-weight-medium">{{
                      new Date(booking.scheduledAt).toLocaleString("fa-IR")
                    }}</span>
                  </template>
                </v-list-item>
                <v-list-item prepend-icon="mdi-map-marker-distance">
                  ...
                  >
                  <template #subtitle
                    >{{ booking.distanceKm }} کیلومتر</template
                  >
                </v-list-item>
                <v-list-item prepend-icon="mdi-account-group">
                  ...
                  >
                  <template #subtitle
                    >{{ booking.passengerCount || 1 }} نفر</template
                  >
                </v-list-item>
                ...
                <v-list-item prepend-icon="mdi-clock-outline">
                  ...
                  >
                  <template #subtitle>{{
                    new Date(booking.createdAt).toLocaleString("fa-IR")
                  }}</template>
                </v-list-item>
              </v-list>
            </v-col>
        ...
        <v-card rounded="xl" class="pa-6">
          <div class="text-subtitle-1 font-weight-bold text-secondary mb-5">
            مراحل رزرو
          </div>
          <div class="timeline">
            <div
              ...
              :class="{
                ...
                cancelled:
                  booking.status === 'cancelled' && step.status === 'cancelled',
              }"
            >
        ...
        <v-card rounded="xl" class="pa-6 mb-4">
          ...
          <div class="d-flex justify-space-between align-center mb-3">
            ...
            <span class="text-body-1 font-weight-bold">{{
              formatToman(booking.estimatedFare)
            }}</span>
          </div>
          <div class="d-flex justify-space-between align-center mb-3">
            <span class="text-body-2 text-grey"
              >مسافت ({{ booking.distanceKm }} کیلومتر)</span
            >
        ...
        <v-card rounded="xl" class="pa-6 mb-4">
          ...
          <div class="d-flex align-center gap-3 mb-4">
            ...
            <div>
              ...
              <div v-if="booking.user?.email" class="text-caption text-grey">
                {{ booking.user.email }}
              </div>
            </div>
          </div>
        ...
        <v-card rounded="xl" class="pa-6 mb-4">
          ...
          <div v-if="booking.driver">
            ...
            <v-list density="compact" class="pa-0 mb-3">
              ...
              >
                <template #title
                  ><span class="text-caption text-grey">خودرو</span></template
                >
                <template #subtitle>
              ...
              >
                <template #title
                  ><span class="text-caption text-grey">پلاک</span></template
                >
                <template #subtitle>
          ...
          <div v-else>
            <div class="text-center py-4">
              ...
              <div class="text-body-2 text-grey mb-4">
                هنوز راننده‌ای تخصیص داده نشده
              </div>
            </div>
            <v-btn
        ...
        <v-card rounded="xl" class="pa-6">
          ...
          <div class="d-flex flex-column gap-2">
            ...
            <div
              ...
              class="text-center text-grey text-body-2 py-2"
            >
    ...
    <v-dialog v-model="assignDialog" max-width="520" persistent>
      <v-card rounded="xl" class="pa-6" style="direction: rtl">
        ...
        >
          ...
          <strong>{{
            booking ? new Date(booking.scheduledAt).toLocaleString("fa-IR") : ""
          }}</strong>
          نمایش داده می‌شوند
        ...
        <div
          ...
          class="d-flex flex-column align-center pa-8 gap-3"
        >
        ...
        <div v-else>
          ...
          >
            ...
            <div class="text-caption">
              تمام رانندگان فعال در این بازه زمانی رزرو دارند. بازه زمانی را در
              تنظیمات کاهش دهید یا رزرو را به زمان دیگری تغییر دهید.
            </div>
          </v-alert>
          ...
          <div v-else>
            ...
            <div style="max-height: 280px; overflow-y: auto" class="mb-4">
              <v-radio-group v-model="selectedDriverId" hide-details>
                ...
                <div
                  v-if="filteredAvailableDrivers.length === 0"
                  class="text-center pa-4 text-grey text-body-2"
                >
                  راننده‌ای یافت نشد
                </div>
              </v-radio-group>
...
const doAssign = async () => {
  ...
  try {
    const { data } = await $api.patch(
      `/bookings/${booking.value.id}/assign-driver`,
      {
        driverId: selectedDriverId.value,
        finalFare: finalFareInput.value
          ? parseFloat(finalFareInput.value)
          : undefined,
      }
    );
    booking.value = data;
...
const statusTimeline = [
  {
    status: "pending",
    label: "رزرو دریافت شد",
    icon: "mdi-clock-outline",
    desc: "مشتری رزرو را ثبت کرده",
  },
  {
    status: "confirmed",
    label: "راننده تخصیص یافت",
    icon: "mdi-check-circle",
    desc: "راننده تأیید و پیامک ارسال شد",
  },
  {
    status: "in_progress",
    label: "سفر در جریان است",
    icon: "mdi-car",
    desc: "راننده مشتری را سوار کرده",
  },
  {
    status: "completed",
    label: "سفر پایان یافت",
...
const isStatusDone = (status: string) => {
  ...
  return (
    statusOrder.indexOf(status) <= statusOrder.indexOf(booking.value.status)
  );
};
...
const doUpdateStatus = async (status: string) => {
  ...
  try {
    const { data } = await $api.patch(`/bookings/${booking.value.id}/status`, {
      status,
    });
    booking.value = data;
  } finally {
...
const openEditFare = () => {
  newFare.value = String(
    booking.value.finalFare || booking.value.estimatedFare || ""
  );
