  ...
  <div>
    <div class="d-flex align-center justify-space-between mb-6">
      <div>
        ...
        <div class="text-body-2 text-grey">مدیریت رزروهای مشتریان</div>
      </div>
    </div>
    ...
    <v-card rounded="xl" class="pa-0">
      ...
      >
        ...
        <template #item.scheduledAt="{ item }">
          <div class="text-body-2">{{ new Date(item.scheduledAt).toLocaleDateString('fa-IR') }}</div>
          <div class="text-caption text-grey">{{ new Date(item.scheduledAt).toLocaleTimeString('fa-IR') }}</div>
        </template>
        ...
        <template #item.driver="{ item }">
          ...
          <v-chip v-else size="small" color="warning" variant="tonal">تعیین نشده</v-chip>
        </template>

        <template #item.estimatedFare="{ item }">
          <span class="font-weight-bold text-primary">تومان{{ (item.finalFare || item.estimatedFare) }}</span>
        </template>
        ...
        <template #item.actions="{ item }">
          ...
          <v-btn v-if="!item.driver && item.status === 'pending'" icon="mdi-account-plus" size="small" variant="text" color="primary" @click.stop="openAssign(item)" />
        </template>
      </v-data-table>
    </v-card>
      ...
      <div v-if="selectedBooking" class="pa-6">
        ...
        <v-list lines="two" density="compact" class="mb-4">
          ...
          <v-list-item prepend-icon="mdi-currency-usd">
            ...
            <template #subtitle>
              <span class="text-primary font-weight-bold">تومان{{ (selectedBooking.finalFare || selectedBooking.estimatedFare) }}</span>
    ...
    <v-dialog v-model="assignDialog" max-width="480">
      <v-card rounded="xl" class="pa-6">
        <div class="text-h6 font-weight-bold text-secondary mb-1">تعیین راننده</div>
        <div class="text-body-2 text-grey mb-5">رزرو #{{ assignTarget?.id?.slice(0,8).toUpperCase() }}</div>
        ...
        <v-text-field v-model="finalFareInput" label="کرایه نهایی (اختیاری)" prepend-inner-icon="mdi-currency-usd" type="number" variant="outlined" rounded="lg" hide-details density="compact" class="mb-5" />
        ...
        <div class="d-flex gap-3" style="flex-direction: column">
          ...
          <v-btn color="primary" block :loading="assigning" :disabled="!selectedDriverId" @click="doAssign">تعیین راننده</v-btn>
        </div>
      </v-card>
    </v-dialog>
  </div>
...
onMounted(async () => {
  await Promise.all([bookingsStore.fetchAllBookings(), loadDrivers()])
})
...
const filteredBookings = computed(() =>
  statusFilter.value === 'all' ? bookingsStore.allBookings : bookingsStore.allBookings.filter((b) => b.status === statusFilter.value),
)

const filteredDrivers = computed(() =>
  drivers.value.filter((d) =>
    !driverSearch.value || d.fullName.toLowerCase().includes(driverSearch.value.toLowerCase()) || d.carPlate.toLowerCase().includes(driverSearch.value.toLowerCase()),
  ),
)

const statusCount = (val: string) =>
  val === 'all' ? bookingsStore.allBookings.length : bookingsStore.allBookings.filter((b) => b.status === val).length

const headers = [
  { title: 'مشتری', key: 'user.fullName', sortable: true },
  { title: 'مسیر', key: 'pickupAddress', sortable: false },
  { title: 'زمان', key: 'scheduledAt', sortable: true },
...
const statusActions = (status: string) =>
  ({
    ...
    in_progress: [{ status: 'completed', label: 'اتمام سفر', color: 'success' }],
...
const openAssign = (booking: any) => {
  assignTarget.value = booking; selectedDriverId.value = ''; finalFareInput.value = ''; assignDialog.value = true
}

const doAssign = async () => {
  assigning.value = true
  try {
    await bookingsStore.assignDriver(assignTarget.value.id, selectedDriverId.value, finalFareInput.value ? parseFloat(finalFareInput.value) : undefined)
