  ...
  <div>
      ...
      <div v-if="selectedBooking" class="pa-6">
        ...
        <v-list lines="two" density="compact" class="mb-4">
          ...
          <v-list-item prepend-icon="mdi-currency-usd">
            ...
            <template #subtitle>
              <span class="text-primary font-weight-bold">${{ (selectedBooking.finalFare || selectedBooking.estimatedFare)?.toFixed(2) }}</span>
    ...
    <v-dialog v-model="assignDialog" max-width="480">
      <v-card rounded="xl" class="pa-6">
        ...
        <div class="text-body-2 text-grey mb-5">Booking #{{ assignTarget?.id?.slice(0,8).toUpperCase() }}</div>
        ...
        <v-text-field v-model="finalFareInput" label="Final Fare override (optional)" prepend-inner-icon="mdi-currency-usd" type="number" variant="outlined" rounded="lg" hide-details density="compact" class="mb-5" />

        <div class="d-flex gap-3">
          <v-btn variant="outlined" color="secondary" block @click="assignDialog = false">Cancel</v-btn>
          <v-btn color="primary" block :loading="assigning" :disabled="!selectedDriverId" @click="doAssign">Assign Driver</v-btn>
        </div>
      </v-card>
    </v-dialog>
  </div>
