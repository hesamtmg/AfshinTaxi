  ...
  <div style="direction: rtl">
    ...
    <v-container>
      ...
      <div v-else-if="filteredBookings.length === 0" class="text-center pa-16">
        <v-icon size="72" color="grey-lighten-2" class="mb-4"
        ...
        <v-btn color="primary" to="/client/booking" prepend-icon="mdi-plus"
          >رزرو سفر جدید</v-btn
        >
      </div>
            ...
            <div class="pa-5">
              ...
              <div class="d-flex align-start gap-3 mb-4">
                ...
                <div class="flex-1 min-w-0">
                  <div
                    class="text-body-2 font-weight-medium text-secondary text-truncate mb-2"
                  >
                    {{ booking.pickupAddress }}
                  </div>
                  <div
                    class="text-body-2 font-weight-medium text-secondary text-truncate"
                  >
              ...
              <div class="d-flex align-center justify-space-between">
                ...
                >
                  ...
                  />
                  <v-tooltip
                    v-if="isCancellationBlocked(booking)"
                    location="top"
                  >
    ...
    >
      <div v-if="selectedBooking" class="pa-6">
          ...
          <div class="d-flex align-center gap-3">
            ...
            <div>
              ...
              <div class="text-caption">
                شماره رزرو: #{{ selectedBooking.id.slice(0, 8).toUpperCase() }}
              </div>
            </div>
          </div>
        </v-card>
        ...
        <v-alert
          v-if="
            isCancellationBlocked(selectedBooking) &&
        ...
        >
          ...
          <div class="text-caption">
            این سفر در بازه {{ cancellationDeadline }} دقیقه‌ای قرار دارد و دیگر
            قابل لغو نیست. برای کمک با پشتیبانی تماس بگیرید.
          </div>
        </v-alert>

        <!-- Driver -->
        <v-card
          v-if="selectedBooking.driver"
        ...
        >
          ...
          <div class="d-flex align-center gap-3">
            <v-avatar color="primary" size="48">
              <v-img
                v-if="selectedBooking.driver?.avatarUrl"
                :src="avatarSrc(selectedBooking.driver?.avatarUrl)"
                cover
              />
        ...
        <v-list lines="two" class="mb-4">
          <v-list-item prepend-icon="mdi-calendar-clock" density="compact">
            <template #title
              ><span class="text-caption text-grey">زمان سفر</span></template
            >
            <template #subtitle>{{
              new Date(selectedBooking.scheduledAt).toLocaleString("fa-IR")
            }}</template>
          </v-list-item>
          <v-list-item prepend-icon="mdi-map-marker-distance" density="compact">
            <template #title
              ><span class="text-caption text-grey">مسافت</span></template
            >
            <template #subtitle
              >{{ selectedBooking.distanceKm }} کیلومتر</template
            >
          </v-list-item>
          <v-list-item prepend-icon="mdi-account-group" density="compact">
            ...
            >
            <template #subtitle
              >{{ selectedBooking.passengerCount || 1 }} نفر</template
            >
          </v-list-item>
          <v-list-item prepend-icon="mdi-cash" density="compact">
            <template #title
              ><span class="text-caption text-grey">کرایه</span></template
            >
            <template #subtitle>
              ...
              <span
                v-if="!selectedBooking.finalFare"
          ...
          >
            <template #title
              ><span class="text-caption text-grey">یادداشت</span></template
            >
        ...
        <div
          v-if="selectedBooking.status === 'pending' && !selectedBooking.driver"
    ...
    <v-dialog v-model="editDialog" max-width="460" persistent>
      <v-card rounded="xl" class="pa-6" style="direction: rtl">
        ...
        >
          <div class="text-caption">
            فقط زمان، تعداد مسافران و یادداشت قابل تغییر است. مسیر سفر قابل
            ویرایش نیست — برای تغییر مسیر رزرو را لغو و دوباره ثبت کنید.
          </div>
        </v-alert>
        ...
        <v-form ref="editForm" v-model="editValid" @submit.prevent="saveEdit">
          ...
          <v-card color="grey-lighten-5" rounded="lg" class="pa-3 mb-4">
            <div class="d-flex align-start gap-3">
              ...
              <div>
                <div
                  class="text-caption font-weight-medium text-secondary mb-2"
                >
                  {{ editTarget?.pickupAddress }}
                </div>
                <div class="text-caption font-weight-medium text-secondary">
                  {{ editTarget?.dropoffAddress }}
                </div>
              </div>
            </div>
            ...
            <div class="d-flex justify-space-between text-caption">
              ...
              <span class="font-weight-bold"
                >{{ editTarget?.distanceKm }} کیلومتر</span
              >
            </div>
            <div class="d-flex justify-space-between text-caption mt-1">
              <span class="text-grey">کرایه تخمینی</span>
              <span class="font-weight-bold text-primary">{{
                formatToman(editTarget?.estimatedFare)
              }}</span>
            </div>
          </v-card>
      ...
      <v-card rounded="xl" class="pa-6" style="direction: rtl">
        ...
        <div class="d-flex gap-3 flex-col">
          <v-btn
            ...
            >نگه داشتن</v-btn
          >
          <v-btn color="error" block :loading="cancelling" @click="doCancel"
            >لغو رزرو</v-btn
          >
        </div>
      </v-card>
...
const ratingBookingId = ref(null);
onMounted(async () => {
  await bookingsStore.fetchMyBookings();
  await loadSettings();
});
const promptRating = async (bookingId) => {
  const { data } = await $api(`/ratings/status/${bookingId}`);
...
const onRated = (stars) => {
  // Optionally show reward notification, refresh trip list, etc.
};
// ── Settings ──────────────────────────────────────────────────────────────────
...
const isCancellationBlocked = (booking: any) => {
  ...
  return (
    (new Date(booking.scheduledAt).getTime() - Date.now()) / (1000 * 60) <
...
const tabCount = (val: string) =>
  val === "all"
    ? 0
    : bookingsStore.myBookings.filter((b) => b.status === val).length;
...
const statusGradient = (s: string) =>
  ({
    pending: "linear-gradient(90deg,#FF9800,#FFB74D)",
    confirmed: "linear-gradient(90deg,#2196F3,#64B5F6)",
    in_progress: "linear-gradient(90deg,#F5A623,#FFD180)",
    completed: "linear-gradient(90deg,#4CAF50,#81C784)",
    cancelled: "linear-gradient(90deg,#F44336,#E57373)",
  }[s] || "#e0e0e0");
