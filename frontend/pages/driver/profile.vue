    ...
    <v-container style="max-width: 760px">
        ...
        <div class="d-flex align-center gap-4 flex-wrap">
          ...
          <div class="flex-1">
            ...
            <div class="text-body-2 text-grey mb-2">
              {{ driverStore.driver?.phone }}
            </div>
            <div class="d-flex gap-2 flex-wrap">
              <v-chip size="small" color="success" label>
                <v-icon start size="14">mdi-check-circle</v-icon>
                {{ getDriverStatusText(driverStore.driver?.status) }}
              </v-chip>
      ...
      <v-row>
          ...
          <v-card rounded="xl" class="pa-5 h-100">
            ...
            <v-list density="compact" class="pa-0">
              ...
              <v-list-item prepend-icon="mdi-phone" class="px-0">
                <template #title
                  ><span class="text-caption text-grey">تلفن</span></template
                >
                <template #subtitle>{{ driverStore.driver?.phone }}</template>
              </v-list-item>
              <v-list-item prepend-icon="mdi-email" class="px-0">
                <template #title
                  ><span class="text-caption text-grey">ایمیل</span></template
                >
                <template #subtitle>{{
                  driverStore.driver?.email || "—"
                }}</template>
              </v-list-item>
              <v-list-item prepend-icon="mdi-card-account-details" class="px-0">
                <template #title
                  ><span class="text-caption text-grey"
                    >شماره گواهینامه</span
                  ></template
                >
                <template #subtitle>{{
                  driverStore.driver?.licenseNumber || "—"
                }}</template>
              </v-list-item>
            </v-list>
          </v-card>
        ...
        <v-col cols="12" md="6">
          <v-card rounded="xl" class="pa-5 h-100">
            <div class="text-subtitle-1 font-weight-bold text-secondary mb-4">
              <v-icon color="primary" class="ml-2">mdi-car</v-icon>
              اطلاعات خودرو
            </div>
            <v-list density="compact" class="pa-0">
              <v-list-item prepend-icon="mdi-car" class="px-0">
                <template #title
                  ><span class="text-caption text-grey">مدل</span></template
                >
                <template #subtitle>{{
                  driverStore.driver?.carModel
                }}</template>
              ...
              <v-list-item prepend-icon="mdi-palette" class="px-0">
                <template #title
                  ><span class="text-caption text-grey">رنگ</span></template
                >
                <template #subtitle>{{
      ...
      <v-card rounded="xl" class="pa-5 mt-4">
        <div class="text-subtitle-1 font-weight-bold text-secondary mb-4">
          <v-icon color="primary" class="ml-2">mdi-chart-bar</v-icon>
          آمار من
        </div>
        <v-row>
          <v-col
      ...
      <v-alert type="info" variant="tonal" rounded="xl" class="mt-4">
        <div class="text-body-2">
          برای به‌روزرسانی اطلاعات پروفایل یا خودرو، لطفاً با ادمین یا
          dispatcher خود تماس بگیرید.
        </div>
      </v-alert>
    </v-container>
  </div>
...
const statCards = computed(() => [
  { label: 'کل سفرها',   value: driverStore.stats?.total     ?? '۰', color: 'secondary' },
  { label: 'تکمیل شده',     value: driverStore.stats?.completed ?? '۰', color: 'success' },
  { label: 'پیش رو',      value: driverStore.stats?.upcoming  ?? '۰', color: 'primary' },
  { label: 'درآمد',      value: `${driverStore.stats?.totalEarnings?.toLocaleString() ?? '۰'} تومان`, color: 'primary' },
])
</script>
